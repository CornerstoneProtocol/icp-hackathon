...
import { getAllProjects, getCompleteProjectData } from '@/lib/canister-queries';
...
const AllProjects = () => {
            ...
            try {
              ...
              if (projectData) {
                return {
                  ...project,
                  projectState: {
                    fundraiseClosed: projectData.fundraiseClosed,
                    fundraiseSuccessful: projectData.fundraiseSuccessful,
              ...
              console.error(`Error fetching state for project ${project.id}:`, error);
            }
          }
          return project;
        })
  ...
  const filteredProjects = projects.filter(project => {
    ...
    const matchesSearch = 
      ...
      project.params.token_symbol.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    if (filterStatus !== 'all') {
      ...
      if (!state) {
        // If no state data, assume it's still in funding (not yet deployed)
        matchesStatus = filterStatus === 'funding';
      } else {
        ...
        <div className="mb-12">
          ...
          <div className="mb-6 bg-[#5599FF]/20 border-4 border-[#5599FF] p-4 max-w-2xl w-full">
            <p className="text-sm text-[#2D1B00] font-semibold break-words">
              💡 <strong>Just created a project?</strong> It may take a few moments to fetch project state from canisters. 
              Click the <strong className="text-[#5599FF]">REFRESH</strong> button to check for updates.
            </p>
          </div>
